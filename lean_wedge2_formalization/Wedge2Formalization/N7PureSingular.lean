...
namespace N7PureSingular

variable {k : Type*} [Field k]

/-- The three-dimensional radical factor for the pure singular `n = 7` orbit. -/
abbrev I := Fin 3
...
/-- A `3 × 4` matrix supported only on the last quotient basis vector. -/
def upperRightLast (x y z : k) : Matrix I W k := !![(0 : k), 0, 0, x; 0, 0, 0, y; 0, 0, 0, z]

/-- The embedded pure singular representative in dimension `7`. -/
def rad3PureSingularRep₁ : Matrix V V k :=
...
def FixesRad3PureSingularPairBivector (g : Matrix V V k) : Prop :=
  FixesBivector rad3PureSingularRep₁ g ∧ FixesBivector rad3PureSingularRep₂ g

/-- Setwise stabilizer of the embedded pure singular `2`-space in the chosen basis. -/
def PreservesRad3PureSingularSubspaceBivector (g : Matrix V V k) : Prop :=
  ∃ α β γ δ : k,
    ActBivector rad3PureSingularRep₁ g = α • rad3PureSingularRep₁ + β • rad3PureSingularRep₂ ∧
    ActBivector rad3PureSingularRep₂ g = γ • rad3PureSingularRep₁ + δ • rad3PureSingularRep₂
...
theorem embed_pureSingular_linearCombination
    ...
    Matrix.fromBlocks 0 0 0 (α • (N4PureSingular.ω12 (k := k)) + β • (N4PureSingular.ω13 (k := k))) =
      α • rad3PureSingularRep₁ + β • rad3PureSingularRep₂ := by
  ext i j
  cases i <;> cases j <;>
    simp [rad3PureSingularRep₁, rad3PureSingularRep₂, Matrix.fromBlocks, Matrix.add_apply]
...
theorem fixesRad3PureSingularPair_fromBlocks_zeroUpperRight_iff
    (A : Matrix I I k)
    (C : Matrix W I k)
    (D : Matrix W W k) :
    FixesRad3PureSingularPairBivector (Matrix.fromBlocks A 0 C D) ↔
    ...
    exact
      ⟨(by
          have h₁' :
            ...
            simpa [rad3PureSingularRep₁] using h₁
...
theorem fixesRad3PureSingularPair_lowerRight
    (A : Matrix I I k)
    (B : Matrix I W k)
    (C : Matrix W I k)
    (D : Matrix W W k) :
    FixesRad3PureSingularPairBivector (Matrix.fromBlocks A B C D) →
      N4PureSingular.FixesPureSingularPair D := by
  rintro ⟨h₁, h₂⟩
  refine ⟨?_, ?_⟩
  · ext i j
    have hij := congrArg (fun M => M (Sum.inr i) (Sum.inr j)) h₁
    simpa [FixesBivector, rad3PureSingularRep₁, N4PureSingular.FixesBivector,
...
theorem fixesRad3PureSingularPair_upperRight_shape
    (A : Matrix I I k)
    (B : Matrix I W k)
    (C : Matrix W I k)
    (D : Matrix W W k)
    (h : FixesRad3PureSingularPairBivector (Matrix.fromBlocks A B C D)) :
    B 0 0 = 0 ∧ B 0 1 = 0 ∧ B 0 2 = 0 ∧
    B 1 0 = 0 ∧ B 1 1 = 0 ∧ B 1 2 = 0 ∧
    B 2 0 = 0 ∧ B 2 1 = 0 ∧ B 2 2 = 0 := by
  have hD : N4PureSingular.FixesPureSingularPair D :=
    fixesRad3PureSingularPair_lowerRight (A := A) (B := B) (C := C) (D := D) h
  rcases
  ...
  have h₁tr : B * (N4PureSingular.ω12 (k := k)) * Dᵀ = 0 := by
    ...
    simpa [FixesBivector, rad3PureSingularRep₁, Matrix.fromBlocks_transpose,
  ...
  rcases hrow_shape 2 with ⟨h200, h201, h202⟩
  exact ⟨h000, h001, h002, h100, h101, h102, h200, h201, h202⟩

lemma upperRightLast_mul_ω12_zero (x y z : k) :
    upperRightLast (k := k) x y z * N4PureSingular.ω12 (k := k) = 0 := by
  ext i j
  fin_cases i <;> fin_cases j <;>
    simp [upperRightLast, N4PureSingular.ω12, Matrix.mul_apply, Fin.sum_univ_four]

lemma upperRightLast_mul_ω13_zero (x y z : k) :
    upperRightLast (k := k) x y z * N4PureSingular.ω13 (k := k) = 0 := by
  ext i j
  fin_cases i <;> fin_cases j <;>
    simp [upperRightLast, N4PureSingular.ω13, Matrix.mul_apply, Fin.sum_univ_four]

lemma ω12_mul_upperRightLast_transpose_zero (x y z : k) :
    N4PureSingular.ω12 (k := k) * (upperRightLast (k := k) x y z)ᵀ = 0 := by
  have h := congrArg Matrix.transpose (upperRightLast_mul_ω12_zero (k := k) x y z)
  have htr : (N4PureSingular.ω12 (k := k))ᵀ * (upperRightLast (k := k) x y z)ᵀ = 0 := by
  ...
  have hneg : -(N4PureSingular.ω12 (k := k) * (upperRightLast (k := k) x y z)ᵀ) = 0 := by
    simpa [hω, neg_mul] using htr
  exact neg_eq_zero.mp hneg

lemma ω13_mul_upperRightLast_transpose_zero (x y z : k) :
    N4PureSingular.ω13 (k := k) * (upperRightLast (k := k) x y z)ᵀ = 0 := by
  have h := congrArg Matrix.transpose (upperRightLast_mul_ω13_zero (k := k) x y z)
  have htr : (N4PureSingular.ω13 (k := k))ᵀ * (upperRightLast (k := k) x y z)ᵀ = 0 := by
    simpa [Matrix.transpose_mul] using h
  have hω :
      (N4PureSingular.ω13 (k := k))ᵀ = -(N4PureSingular.ω13 (k := k)) := by
    ext i j
    fin_cases i <;> fin_cases j <;> simp [N4PureSingular.ω13]
  have hneg : -(N4PureSingular.ω13 (k := k) * (upperRightLast (k := k) x y z)ᵀ) = 0 := by
    simpa [hω, neg_mul] using htr
  exact neg_eq_zero.mp hneg
...
theorem rad3PureSingular_pointwise_family_with_upperRight
    (a b c d e f t x y z : k)
    (ha : a ≠ 0)
    (A0 : Matrix I I k)
    (C : Matrix W I k) :
    FixesRad3PureSingularPairBivector
      (Matrix.fromBlocks
        A0
        (upperRightLast (k := k) x y z)
        C
  ...
  ·
    calc
      ActBivector rad3PureSingularRep₁
          (Matrix.fromBlocks
            A0
            (upperRightLast (k := k) x y z)
            C
            ...
            (N4PureSingular.pureSingularShape (k := k) a b c d e f t)) := by
              simpa [rad3PureSingularRep₂] using
                act_embedded_fromBlocks_annihilating_upperRight
                  ...
                  (C := C)
                  (D := N4PureSingular.pureSingularShape (k := k) a b c d e f t)
                  (hleft := upperRightLast_mul_ω13_zero (k := k) x y z)
                  (hright := ω13_mul_upperRightLast_transpose_zero (k := k) x y z)
      _ = Matrix.fromBlocks 0 0 0 (N4PureSingular.ω13 (k := k)) := by
            simpa [N4PureSingular.FixesBivector] using hfix.2
      _ = rad3PureSingularRep₂ := by simp [rad3PureSingularRep₂]

/-- Sharp pointwise description for the pure singular radical extension in dimension `7`. -/
theorem fixesRad3PureSingularPair_fromBlocks_iff
    (A : Matrix I I k)
    (B : Matrix I W k)
    (C : Matrix W I k)
    (D : Matrix W W k) :
    FixesRad3PureSingularPairBivector (Matrix.fromBlocks A B C D) ↔
      B = upperRightLast (k := k) (B 0 3) (B 1 3) (B 2 3) ∧
      D 0 0 ≠ 0 ∧
      D =
  ...
  · intro h
    ...
    rcases
      (N4PureSingular.fixesPureSingularPair_iff_shape (k := k) (g := D)).1 hD with
      ⟨hD00, hshape⟩
    rcases fixesRad3PureSingularPair_upperRight_shape
        (A := A) (B := B) (C := C) (D := D) h with
      ⟨h000, h001, h002, h100, h101, h102, h200, h201, h202⟩
    refine ⟨?_, hD00, hshape⟩
    ext i j
    fin_cases i <;> fin_cases j <;>
      simp [upperRightLast, h000, h001, h002, h100, h101, h102, h200, h201, h202]
  · rintro ⟨hB, hD00, hshape⟩
    rw [hB, hshape]
    exact
      rad3PureSingular_pointwise_family_with_upperRight
        (k := k)
        (a := D 0 0)
        (b := D 0 1)
        (c := D 0 2)
...
pure singular orbit. -/
theorem rad3PureSingular_pointwise_family
    (a b c d e f t : k)
    (ha : a ≠ 0)
    (A0 : Matrix I I k)
    (C : Matrix W I k) :
  ...
  have hD :
    ...
    N4PureSingular.pureSingularShape_fixes (k := k) a b c d e f t ha
  exact
    (fixesRad3PureSingularPair_fromBlocks_zeroUpperRight_iff
      (k := k)
      (A := A0)
      (C := C)
...
vanishes. -/
theorem preservesRad3PureSingularSubspace_fromBlocks_zeroUpperRight
    (A : Matrix I I k)
    (C : Matrix W I k)
    (D : Matrix W W k)
    (hD : N4PureSingular.PreservesPureSingularSubspaceBivector D) :
    PreservesRad3PureSingularSubspaceBivector (Matrix.fromBlocks A 0 C D) := by
  rcases hD with ⟨α, β, γ, δ, h₁, h₂⟩
  refine ⟨α, β, γ, δ, ?_, ?_⟩
  ·
    calc
      ActBivector rad3PureSingularRep₁ (Matrix.fromBlocks A 0 C D)
      ...
      _ = α • rad3PureSingularRep₁ + β • rad3PureSingularRep₂ := by
  ...
  ·
    calc
      ...
      _ = γ • rad3PureSingularRep₁ + δ • rad3PureSingularRep₂ := by
            exact embed_pureSingular_linearCombination (k := k) (α := γ) (β := δ)

/-- The pure singular setwise family extends directly to the three-dimensional radical
extension. -/
theorem rad3PureSingular_setwise_family
    (a b c d p q r s e f t : k)
    (A0 : Matrix I I k)
    (C : Matrix W I k) :
    PreservesRad3PureSingularSubspaceBivector
      (Matrix.fromBlocks
        A0
        0
        C
        (N4PureSingular.pureSingularSetwiseShape (k := k) a b c d p q r s e f t)) := by
  apply preservesRad3PureSingularSubspace_fromBlocks_zeroUpperRight
...
theorem rad3PureSingular_GL2_lift_action
    ...
    ActBivector rad3PureSingularRep₁ g =
        α • rad3PureSingularRep₁ + β • rad3PureSingularRep₂ ∧
      ActBivector rad3PureSingularRep₂ g =
        γ • rad3PureSingularRep₁ + δ • rad3PureSingularRep₂ := by
  dsimp
