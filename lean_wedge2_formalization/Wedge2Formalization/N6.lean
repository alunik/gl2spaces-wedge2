...
/-- The two-dimensional radical factor used for the first `n = 6` orbit. -/
abbrev I := Fin 2
...
def FixesRad2SplitPairBivector (g : Matrix V V k) : Prop :=
  FixesBivector rad2SplitRep₁ g ∧ FixesBivector rad2SplitRep₂ g

/-- Setwise stabilizer of the embedded split-support `2`-space in the chosen basis. -/
def PreservesRad2SplitSubspaceBivector (g : Matrix V V k) : Prop :=
  ∃ α β γ δ : k,
    ActBivector rad2SplitRep₁ g = α • rad2SplitRep₁ + β • rad2SplitRep₂ ∧
    ActBivector rad2SplitRep₂ g = γ • rad2SplitRep₁ + δ • rad2SplitRep₂
...
theorem embed_split_linearCombination
    ...
    Matrix.fromBlocks 0 0 0 (α • (N4.splitRep₁ (k := k)) + β • (N4.splitRep₂ (k := k))) =
      α • rad2SplitRep₁ + β • rad2SplitRep₂ := by
  ext i j
  cases i <;> cases j <;>
    simp [rad2SplitRep₁, rad2SplitRep₂, Matrix.fromBlocks, Matrix.add_apply]
...
/-- The pairwise version of the lower-right reduction for the two-dimensional radical
extension. -/
theorem fixesRad2SplitPair_fromBlocks_zeroUpperRight_iff
    (A : Matrix I I k)
    (C : Matrix W I k)
    (D : Matrix W W k) :
    FixesRad2SplitPairBivector (Matrix.fromBlocks A 0 C D) ↔
...
theorem fixesRad2SplitPair_lowerRight
    (A : Matrix I I k)
    (B : Matrix I W k)
    (C : Matrix W I k)
    (D : Matrix W W k) :
    FixesRad2SplitPairBivector (Matrix.fromBlocks A B C D) →
      N4.FixesSplitPairBivector D := by
  rintro ⟨h₁, h₂⟩
  refine ⟨?_, ?_⟩
  · ext i j
    have hij := congrArg (fun M => M (Sum.inr i) (Sum.inr j)) h₁
    simpa [FixesBivector, rad2SplitRep₁, N4.FixesBivector,
...
vanishes. -/
theorem preservesRad2SplitSubspace_fromBlocks_zeroUpperRight
    (A : Matrix I I k)
    (C : Matrix W I k)
    (D : Matrix W W k)
    (hD : N4.PreservesSplitSubspaceBivector D) :
    PreservesRad2SplitSubspaceBivector (Matrix.fromBlocks A 0 C D) := by
  rcases hD with ⟨α, β, γ, δ, h₁, h₂⟩
  refine ⟨α, β, γ, δ, ?_, ?_⟩
  ·
    calc
      ActBivector rad2SplitRep₁ (Matrix.fromBlocks A 0 C D)
          = Matrix.fromBlocks 0 0 0 (N4.ActBivector (N4.splitRep₁ (k := k)) D) := by
              simpa [rad2SplitRep₁] using
      ...
      _ = α • rad2SplitRep₁ + β • rad2SplitRep₂ := by
            ext i j
            cases i <;> cases j <;>
              simp [rad2SplitRep₁, rad2SplitRep₂, Matrix.fromBlocks, Matrix.add_apply]
  ·
    calc
      ...
      _ = γ • rad2SplitRep₁ + δ • rad2SplitRep₂ := by
            ext i j
            cases i <;> cases j <;>
              simp [rad2SplitRep₁, rad2SplitRep₂, Matrix.fromBlocks, Matrix.add_apply]

/-- The two-dimensional radical extension of the split-support pointwise stabilizer. The
radical block is free, the radical columns are free, and the lower-right block is the
pointwise stabilizer from the `n = 4` split-support orbit. -/
theorem rad2Split_pointwise_family
    (A0 : Matrix I I k)
    (C : Matrix W I k)
    (A E : Matrix N4.I N4.I k)
    (hA : A.det = 1)
    (hE : E.det = 1) :
    FixesRad2SplitPairBivector
  ...
  have hD : N4.FixesSplitPairBivector (Matrix.fromBlocks A 0 0 E) := by
    exact
      (N4Summary.split_pointwise_bivector_iff (k := k) (A := A) (B := 0) (C := 0) (D := E)).2
        ⟨hA, rfl, rfl, hE⟩
  exact
    (fixesRad2SplitPair_fromBlocks_zeroUpperRight_iff
      (k := k)
      (A := A0)
      (C := C)
      (D := Matrix.fromBlocks A 0 0 E)).2 hD

/-- For the two-dimensional radical extension of the split-support orbit, pointwise
fixing forces the upper-right block to vanish. -/
theorem fixesRad2SplitPair_fromBlocks_iff
    (A : Matrix I I k)
    (B : Matrix I W k)
    (C : Matrix W I k)
    (D : Matrix W W k) :
  ...
  · intro h
    have hD : N4.FixesSplitPairBivector D :=
      fixesRad2SplitPair_lowerRight (A := A) (B := B) (C := C) (D := D) h
    have hDblocks :
    ...
    have h₂tr : B * (N4.splitRep₂ (k := k)) * Dᵀ = 0 := by
      ...
      simpa [FixesBivector, rad2SplitRep₂, Matrix.fromBlocks_transpose,
...
/-- The two-dimensional radical extension also preserves the split-support `2`-space
setwise whenever the lower-right block preserves the underlying split-support `n = 4`
subspace. -/
theorem rad2Split_setwise_family
    (A0 : Matrix I I k)
    (C : Matrix W I k)
    (A D : Matrix N4.I N4.I k) :
    PreservesRad2SplitSubspaceBivector
      (Matrix.fromBlocks A0 0 C (Matrix.fromBlocks A 0 0 D)) := by
  apply preservesRad2SplitSubspace_fromBlocks_zeroUpperRight
...
theorem rad2Split_torus_lift_action
    ...
    ActBivector rad2SplitRep₁ g = u • rad2SplitRep₁ + (v - u) • rad2SplitRep₂ ∧
      ActBivector rad2SplitRep₂ g = v • rad2SplitRep₂ := by
  dsimp
  let A : Matrix N4.I N4.I k := !![u, 0; 0, 1]
  let D : Matrix N4.I N4.I k := !![v, 0; 0, 1]
  ...
  ·
    calc
      ActBivector rad2SplitRep₁ (Matrix.fromBlocks A0 0 C (Matrix.fromBlocks A 0 0 D)) =
      ...
      _ = u • rad2SplitRep₁ + (v - u) • rad2SplitRep₂ := by
  ...
  ·
    calc
      ...
      _ = v • rad2SplitRep₂ := by
            ext i j
            cases i <;> cases j <;>
              simp [rad2SplitRep₂, Matrix.fromBlocks]

/-- The swap coset in the split normalizer also extends directly to the two-dimensional
radical extension. -/
theorem rad2Split_swapCoset_lift_action
    ...
    let g : Matrix V V k := Matrix.fromBlocks A0 0 C h
    ActBivector rad2SplitRep₁ g = u • rad2SplitRep₁ + (v - u) • rad2SplitRep₂ ∧
      ActBivector rad2SplitRep₂ g = u • rad2SplitRep₁ + (-u) • rad2SplitRep₂ := by
  dsimp
  let A : Matrix N4.I N4.I k := !![u, 0; 0, 1]
  let D : Matrix N4.I N4.I k := !![v, 0; 0, 1]
  ...
  ·
    calc
      ...
      _ = u • rad2SplitRep₁ + (-u) • rad2SplitRep₂ := by
